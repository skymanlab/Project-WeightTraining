...
public class RightDataChecker {
    ...
    public static boolean checkWhetherRightEvent(Event event) {
        ...
        if (event != null) {
            ...
            // [check 2] : count 값을 할당 받았다.
            if (0 < event.getCount()) {

                LogManager.displayLog(CLASS_LOG_DISPLAY_POWER, CLASS_NAME, METHOD_NAME, "=> check_2/true : " + event.getCount() + " 번째 데이터로 등록되었습니다. <=");

                // [lv/b]isRightEvent : 너의 데이터가 등록되었어!
                isRightEvent = true;

            } else {
                LogManager.displayLog(CLASS_LOG_DISPLAY_POWER, CLASS_NAME, METHOD_NAME, "=> check_2/false : 데이터베이스에 저장이 안 된 거 같아! <=");
            } // [check 2]
